...
func (p *Proxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
    ...
    // Forward headers immediately to allow streaming
    ...
    // Implement Streaming: MultiWriter writes to client and local buffer simultaneously
    var bodyBuf bytes.Buffer
    mw := io.MultiWriter(w, &bodyBuf)
    ...
    for {
        ...
        if n > 0 {
            _, writeErr := mw.Write(chunk[:n])
            if writeErr != nil {
                log.Printf("ERROR: client write error: %v", writeErr)
                break
            }
            if ok {
    ...
    // Extract metrics from the accumulated buffer after the stream ends
    if worthy && response.StatusCode >= 200 && response.StatusCode < 300 {
        p.safeExtractAndRecord(r.URL.Path, model, bodyBuf.Bytes(), time.Since(startTime))
    } else if !worthy {
...
// BackendHealthy returns the last known backend health
